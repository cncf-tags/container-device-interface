...
func TestSave(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedOutput:      `{"cdiVersion":"v0.3.0","kind":"example.com/class","devices":null,"containerEdits":{"deviceNodes":[{"path":"/dev/foo"}]}}`,
		},
		{
		...
		{
			description: "spec is validated on save",
			spec: cdi.Spec{
				Version: "v99.3.0",
			},
			options: []Option{
				WithSpecValidator(&validatorWithError{errInvalid}),
			},
			filename:      "foo.json",

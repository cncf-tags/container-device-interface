...
func TestAnnotationKey(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		},
		{
			...
			plugin: "vendor.class",
			devID:  "device",
			key:    Prefix + "vendor.class" + "_" + "device",
		},
		{
			name:   "valid, with special characters",
			plugin: "v-e.n_d.or.cl-as_s",
			devID:  "d_e-v-i-c_e",
			key:    Prefix + "v-e.n_d.or.cl-as_s" + "_" + "d_e-v-i-c_e",
		},
		{
			name:   "valid, with /'s replaced in devID",
			plugin: "v-e.n_d.or.cl-as_s",
			devID:  "d-e/v/i/c-e",
			key:    Prefix + "v-e.n_d.or.cl-as_s" + "_" + "d-e_v_i_c-e",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			key, err := Key(tc.plugin, tc.devID)
			if !tc.invalid {
	...
	for _, tc := range []*testCase{
		{
			...
			annotations: map[string]string{
				Prefix + "vendor.class_device": "vendor.com/class=device",
			},
			parsed: []string{
				"vendor.com/class=device",
			},
		},
		{
			...
			annotations: map[string]string{
				Prefix + "vendor.class_device": "vendor.com/class=device1,vendor.com/class=device2,vendor.com/class=device3",
			},
			parsed: []string{
				"vendor.com/class=device1",
				"vendor.com/class=device2",
		...
		},
		{
			...
			annotations: map[string]string{
				Prefix + "vendor1.class_device1": "vendor1.com/class=device1",
				Prefix + "vendor1.class_device2": "vendor2.com/class=device1,vendor2.com/class=device2",
				Prefix + "vendor3.class2_device": "vendor3.com/class2=device1,vendor3.com/class2=device2,vendor3.com/class2=device3",
			},
			parsed: []string{
				"vendor1.com/class=device1",
				"vendor2.com/class=device1",
		...
		},
		{
			...
			existing: map[string]string{
				Prefix + "vendor3.class2_device": "vendor3.com/class2=device0",
			},
			injections: []*inject{
				{
					plugin: "vendor1.class",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, i := range tc.injections {
				...
				annotations, err = Update(annotations, i.plugin, i.devID, i.devices)
				if !tc.invalid {
			...
			if tc.invalid {
				...
				_, parsed, err = Parse(annotations)
	...
	for _, tc := range []*testCase{
		{
			...
			annotations: map[string]string{
				Prefix + "vendor.class_device": "vendor.com/class=device1",
			},
			devices: []string{
				"vendor.com/class=device1",
			},
		...
		},
		{
			name: "multiple plugins, multiple devices",
			annotations: map[string]string{
				Prefix + "vendor1.class_device":  "vendor1.com/class=device1",
				Prefix + "vendor2.class_device":  "vendor2.com/class=device1,vendor2.com/class=device2",
				Prefix + "vendor3.class2_device": "vendor3.com/class2=device1,vendor3.com/class2=device2,vendor3.com/class2=device3",
			},
			devices: []string{
				"vendor1.com/class=device1",
		...
		},
		{
			name: "invalid, malformed device reference",
			annotations: map[string]string{
				Prefix + "vendor1.class1_device": "vendor1.com/class1=device1",
				Prefix + "vendor2.class2_device": "vendor2.com/class2=device2",
				Prefix + "vendor3.class_device":  "vendor3.com=device3",
			},
			invalid: true,
		},
		{
			name: "invalid, pre-resolved device",
			annotations: map[string]string{
				Prefix + "vendor1.class2_device": "vendor1.com/class2=device1,vendor1.com/class2=device2",
				Prefix + "vendor2.class_device":  "/dev/null",
			},
			invalid: true,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, devices, err := Parse(tc.annotations)
			if !tc.invalid {

...
type options struct {
	outputFormat string
	validator    validator
	// The following options are only applicable when calling the Save function.
	overwrite   bool
	permissions os.FileMode
}

// Save a CDI specification to the requested path.
// The file is created at the target path as an atomic operation by first
// creating a temporary file in the target directory. If the path consists of
// only a filename, the current working directory is used as the target
// directory.
//
// A set of options control how the file is to be saved.
//
// The output format (e.g. YAML or JSON) of the spec is determined from the
// provided path based on the file extension and overrides any caller-specified
// format.
//
// Note that it is up to the caller to ensure that the specified path has an
// extension that is consistent with the format. The EnsureExtension helper
...
// WriteTo writes a CDI specification to a writer with the supplied options.
// The specification is marshalled based on the configured output format (YAML
// or JSON) before being writtent to the writer.
// If a validator is supplied, this is applied to the output specification
// before writing.
...
// EnsureExtension returns the specified path with the extension specified by
// the options appended if required.
...
// marshal the specified content based on the configured output format.
func (o *options) marshal(v any) ([]byte, error) {
	...
	case "yaml":
		...
		// We prepend a YAML separator to the output to allow multiple YAML
		// files to be concatenated together if required.
		data = append([]byte("---\n"), data...)
		return data, err
	case "json":
